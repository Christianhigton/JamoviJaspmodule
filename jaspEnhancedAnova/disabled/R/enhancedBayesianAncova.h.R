...
enhancedBayesianAncovaOptions <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "enhancedBayesianAncovaOptions",
...
enhancedBayesianAncovaResults <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "enhancedBayesianAncovaResults",
    ...
    public=list(
        initialize=function(options) {
            super$initialize(
                options=options,
                name="",
                title="JASP Enhanced Bayesian ANCOVA")
...
enhancedBayesianAncovaBase <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "enhancedBayesianAncovaBase",
    ...
    public = list(
        initialize = function(options, data=NULL, datasetId="", analysisId="", revision=0) {
            super$initialize(
                ...
                version = c(0,1,0),
                options = options,
                results = enhancedBayesianAncovaResults$new(options=options),
                data = data,
                datasetId = datasetId,
...
#' JASP Enhanced Bayesian ANCOVA
#'
#' jamovi-native shell for the JASP Bayesian ANCOVA analysis.
#' @param data .
#' @param dependent .
#' @param fixedFactors .
#' @param covariates .
...
#'
#' @export
enhancedBayesianAncova <- function(
    data,
    dependent,
    fixedFactors,
    covariates,
    ...
    if ( ! requireNamespace("jmvcore", quietly=TRUE))
        stop("enhancedBayesianAncova requires jmvcore to be installed (restart may be required)")
    ...
    options <- enhancedBayesianAncovaOptions$new(
        dependent = dependent,
    ...
    analysis <- enhancedBayesianAncovaClass$new(
        options = options,
        data = data)
